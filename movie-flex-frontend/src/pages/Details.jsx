...
const Details = () => {
  ...
  const handleLike = () => {
    ...
    axios
      .post(`/api/movies/${currentMovie.id}/like`)
      .then((response) => {
  ...
  const handleDislike = () => {
    ...
    axios
      .post(`/api/movies/${currentMovie.id}/dislike`)
      .then((response) => {
        const updatedMovie = response.data;

        const updatedMovies = movies.map((movie, index) =>
          index === currentMovieIndex
            ? { ...movie, dislikes: updatedMovie.dislikes }
            : movie
        );
    ...
    <div className="w-full flex justify-center items-center">
      ...
      <div className="mt-8 w-full max-w-2xl">
        ...
        <form className="flex flex-col space-y-2 mt-4">
          ...
          <button
            type="submit"
            className="self-end bg-blue-500 text-white py-2 px-4 rounded-md"
          >
            Post Comment
          </button>
        </form>
